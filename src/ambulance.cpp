...
void ambulance::saveMap() {
  ...
  f << "ambulanceId,model,manufacturer,vrn,idle?,headedTowards(ifNotIdle),"
       "driverID(ifNotIdle)\n";
  for (auto i : hospital::ambulancesList) {
    f << i.second.id << "," << i.second.model << "," << i.second.manufacturer
      << "," << i.second.vrn << ","
...
void ambulance::getDetails(int rec) {
  ...
  // 2: Filter by model;
    ...
    if (matchingRecords.size() > rec) {
      do {
        ...
        cin >> reqId;
        if (hospital::ambulancesList.find(reqId) !=
            hospital::ambulancesList.end())
          *this = hospital::ambulancesList[reqId];
        else {
  ...
  // 3: Filter by vehicle reg. number;
    ...
    // if a record is found, it's details will be stored in the driver class
    // object that called this function, and the control is returned; else:
    cout << "\nNo matching record found!\n";
  }
  return;
}
void ambulance::getDetailsFromHistory() {
  ...
  // 2: Filter by vrn;
    ...
    f.close();
    // if a record is found, it's details will be stored in the ambulance class
    // object that called this function, and the control is returned; else:
    cout << "\nNo matching record found!\n";
  }
  return;
...
void ambulance::send() {
  ...
  cout << model << " by " << manufacturer << " sent with driver " << D.firstName
       << " " << D.lastName << " (ID = " << D.id << ") successfully!\n";
  return;
}
void ambulance::reportArrival() {
  ...
  // updating status of driver;
  // note that if we first update the status of ambulance we will lose the
  // identity of it's driver; and then there will be no way to update the status
  // of the driver;
...
void ambulance::removeAmbulance() {
  ...
  if (id == -1)
    return;
  if (!idle) {
    cout << "\nSorry, the ambulance you selected to remove is NOT currently "
  ...
  cout << model << " by " << manufacturer << " (VRN = " << vrn
