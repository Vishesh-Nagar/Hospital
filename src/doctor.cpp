...
void doctor::saveMap() {
  ...
  f << "doctorId,firstName,lastName,gender,age,mobNumber,address,type,"
       "appointmentsBooked\n";
...
void doctor::printDetails() {
  ...
  person::printDetails();
  cout << "Type            : " << type << "\n";
  cout << "Appointments    : " << appointmentsBooked
       << "/8 (appointments booked today)\n";
  return;
}
...
void doctor::removePerson() {
  ...
  if (appointmentsBooked > 0) {
    cout << "\nSelected doctor has appointments booked for today, can't be "
            "removed.\n\n";
    return;
  }
