...
void driver::saveMap() {
  ...
  f << "driverId,firstName,lastName,gender,age,mobNumber,address,licenseNumber,"
       "idle?\n";
  for (auto i : hospital::driversList)
    f << i.second.id << "," << i.second.firstName << "," << i.second.lastName
      << "," << i.second.gender << "," << i.second.age << ","
      << i.second.mobNumber << "," << i.second.add.addToStr() << ","
...
void driver::printDetailsFromHistory(string extraDetails) {
  ...
  if (extraDetails == "") {
    ...
    while (getline(f >> ws, temp)) {
      ...
      if (d.firstName == firstName && d.lastName == lastName &&
          d.mobNumber == mobNumber) {
        getline(s, s7, ',');
        getline(s, d.licenseNumber, ',');
        getline(s, s1, ',');
        getline(s, s2, ',');
...
void driver::removePerson() {
  ...
  if (id == -1)
    return;
  if (!idle) {
    cout << "\nSorry, the driver you selected to remove is NOT currently "
