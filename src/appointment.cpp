...
void appointment::saveMap() {
    ...
    f << "appointmentId,date(YYYYMMDD),doctorId,patientId,startTime(in 24-hr "
         "format)\n";
    for (auto i : hospital::appointmentsList)
        f << i.second.id << "," << yyyymmdd << "," << i.second.D.id << ","
          << i.second.P.id << "," << i.second.hh << endl;
    f.close();
...
void appointment::printDetails() {
    if (id == -1)
        return;
    cout << "\n\n\nAppointment Details:\nID                 : " << id << "\n"
         << "Patient's Name     : " + P.firstName + " " + P.lastName + "(ID = "
         << P.id << ")\n"
         << "Doctor's Name      : " + D.firstName + " " + D.lastName + "(ID = "
         << D.id << ")\n"
         << "Time (24 Hr format): " << hh << ":00 Hrs to " << hh + 1
         << ":00 Hrs\n\n";
    return;
}
void appointment::book() {
    ...
    while (ans == 'Y') {
        ...
        if (D.id == -1) {
            ...
            cout << "Sorry, selected doctor has no free slot left for the "
                    "day!\n";
    ...
    if (hospital::appointmentsList.rbegin() !=
        hospital::appointmentsList.rend())
    ...
    cout << "\nAppointment of patient " + P.firstName + " " + P.lastName +
                " with doctor "
...
void appointment::getDetails() {
    ...
    cin >> id;
    if (hospital::appointmentsList.find(id) ==
        hospital::appointmentsList.end()) {
