...
void patient::saveMap() {
  ...
  f << "patientId,firstName,lastName,gender,age,mobNumber,address,height,"
       "weight,wasHospitalized?,stillAlive(ifHospitalized)?\n";
...
void patient::addPerson() {
  ...
  f << firstName << "," << lastName << "," << gender << "," << age << ","
    << mobNumber << "," << add.addToStr() << "," << height << "," << weight
    << "," << ((hospitalized) ? "Y" : "N") << ","
    << "Y"
    << ","
    << "N"
...
void patient::reportADeath() {
  ...
  cout << firstName << " " << lastName
       << " reported dead, we hope their soul rests in peace, amen!\n";
  return;
}
